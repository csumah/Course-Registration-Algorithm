...
class RQueue {
     ...
     ******************************************/
    Node* deepCopy(const Node* rhsNode); // helper for assignment operator
    void copyHelp(Node* rhsNode); // helper for copy constructor to copy nodes
    Node* deleteStudent(Node* cNode); // deletes highest priority student
    Node* merge(Node* heap1, Node* heap2); // skew merge function.
    bool empty(); // returns true if the queue is empty.
    void printHelp(Node* cNode) const;
    void clear(Node* cNode); // clear helper.
    void heapRebuild(Node* cNode); // helper to rebuild heap.
    void updateNPL(Node* cNode); // calculates and returns the npl of a given node
    Node* buildMinHeap(Node* cNode);
    Node* buildMaxHeap(Node* cNode);
};
#endif